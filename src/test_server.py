"""
サーボ無しで Wi-Fi 接続 + HTTP サーバだけ動作確認するテスト用スクリプト。
Pico WH に config.py と一緒に転送して実行する。

確認項目:
  1. Wi-Fi に接続できるか（シリアルに IP が表示される）
  2. スマホのブラウザで http://<IP>/ を開けるか
  3. 「開錠」ボタンを押して /unlock の応答が返るか
     （シリアルに "[TEST] press_button called" と出れば成功）
"""
...
<title>Interphone Unlocker (TEST)</title>
<style>
...
async function doUnlock(){
  ...
  try{
    ...
    if(r.ok){msg.textContent='OK: '+j.status; msg.className='msg ok';}
    else{msg.textContent='Error: '+r.status; msg.className='msg err';}
  }catch(e){msg.textContent='通信失敗: '+e; msg.className='msg err';}
...
def start_server(servo):
    ...
    while True:
        ...
        try:
            ...
            if path == "/unlock":
                servo.press_button()
                send_response(cl, 200, "application/json", '{"status":"ok (test)"}')
            elif path == "/health":
